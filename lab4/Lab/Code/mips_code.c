...
void trans_ARITH(InterCode cur_instr, FILE* fp) {
	...
	char cur_regName_z[12];
	memset(cur_regName_x, 0, 12);
	memset(cur_regName_y, 0, 12);
	memset(cur_regName_z, 0, 12);
	if(leftOp->kind == TADDRI) {
		sprintf(str, "\tlw $s3, 0($%s)\n",regName(y));  fputs(str, fp); memset(str, 0, INSTR_LEN);
		strcpy(cur_regName_y, "s3");
	}
	else 
		{strcpy(cur_regName_y, regName(y));}

	if(rightOp->kind == TADDRI) {
		sprintf(str, "\tlw $s4, 0($%s)\n",regName(z));  fputs(str, fp); memset(str, 0, INSTR_LEN);
		strcpy(cur_regName_z, "s4");
	}
	else 
		{strcpy(cur_regName_z, regName(z));}

	if(result->kind == TADDRI) {
		sprintf(str, "\tlw $s5, 0($%s)\n",regName(z));  fputs(str, fp); memset(str, 0, INSTR_LEN);
		strcpy(cur_regName_x, "s5");
	}
	else 
		{strcpy(cur_regName_x, regName(x));}
	switch (cur_instr->kind) {
		case OP_PLUS:
			sprintf(str, "\tadd $%s, $%s, $%s\n", cur_regName_x, cur_regName_y, cur_regName_z);		fputs(str, fp);
			break;
		case OP_MINUS:
		...
		case OP_STAR:
			...
			break;
		case OP_DIV:
			sprintf(str, "\tdiv $%s, $%s\n", cur_regName_y, cur_regName_z);	fputs(str, fp);	memset(str, 0, sizeof(str));
			sprintf(str,"\tmflo $%s\n", regName(x));					fputs(str, fp);
			break;
	}
...
void trans_RETURN(InterCode cur_instr, FILE* fp) {  
	...
	if(op->kind != CONSTI){
		...
		sprintf(str, "\taddi $sp, $sp, %d\n", ginfo.stack_size);		fputs(str, fp); memset(str, 0, sizeof(str));
		sprintf(str, "\taddi $sp, $sp, %d\n", 4);				fputs(str, fp); memset(str, 0, sizeof(str));
...
void trans_FUNCTION(InterCode cur_instr, FILE* fp){ //int ginfo.stack_size = 200;
	...
	fputs("\tmove $fp, $sp\n", fp); 
	fputs("\taddi $sp, $sp, -4\n", fp);
	sprintf(str, "\tsubu $sp, $sp, %d\n", ginfo.stack_size);			fputs(str, fp);

	ginfo.sp_offset = 0;
	ginfo.cur_param = 0;
	ginfo.fst_param = true;
}
...
void trans_ARG(InterCode cur_instr, FILE* fp){
	...
	if(op->kind == TMPVARI){
		...
		arg = find_var(argName);
	} 
	else if(op->kind == VARI){
		arg = find_var(op->u.value);
	}
...
void trans_PARAM(InterCode cur_instr, FILE* fp){ //printf("aaaaaaaaauusuudfusdufusufdufsufsufu\n");
	if(ginfo.fst_param) { //printf("aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa");
		ginfo.fst_param = false;
		InterCode fst_param = cur_instr;
		int pnum = 0;
		while(fst_param->kind == OP_PARAM) { 
			pnum++;
			fst_param = fst_param->next;
		}
		ginfo.param_num = pnum;
 	}
	char str[INSTR_LEN];
	memset(str, 0, sizeof(str));
	...
	add_var(param);
	if(ginfo.param_num <= 4) { //printf("%d<<====arg %d<===param",ginfo.cur_arg, ginfo.cur_param);
		sprintf(str, "\tsw $a%d, %d($fp)\n", ginfo.param_num - ginfo.cur_param - 1, param->offset);	fputs(str, fp);
		++ginfo.cur_param;		
	}
	else {
		if(ginfo.param_num - ginfo.cur_param <= 4) {
			sprintf(str, "\tsw $a%d, %d($fp)\n", ginfo.param_num - ginfo.cur_param - 1, param->offset);	fputs(str, fp);
			//++ginfo.cur_param;
		}
		else {
			sprintf(str, "\tlw $v1, %d($fp)\n", (ginfo.cur_param + 1) *4);		fputs(str, fp); memset(str, 0, INSTR_LEN);
			sprintf(str, "\tsw $v1, %d($fp)\n", param->offset);					fputs(str, fp);	
		}
		++ginfo.cur_param;
	}
...
void trans_GETADDR(InterCode cur_instr, FILE* fp){
	...
	if(rightOp->kind == TMPVARI){
		...
		arrayHead = find_var(arrayName);
	} else if(rightOp->kind == VARI){
		arrayHead = find_var(rightOp->u.value);
	}
	if(arrayHead == NULL){
		assert(0);
	}
	...
	char str[INSTR_LEN];
	memset(str, 0, sizeof(str));
	sprintf(str, "\tla $%s, %d($fp)\n", regName(x), arrayHead->offset);		fputs(str, fp);
	store_word(x, fp);
}
